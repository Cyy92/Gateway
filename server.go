...
func (f *FxGateway) InvokeVM(c context.Context, s *pb.InvokeServiceRequest) (*pb.Message, error) {
	start := time.Now()
	output, err := cmd.InvokeVM(s.Service, f.conf.VMNamespace, f.conf.FxWatcherPort, s.Input, f.conf.InvokeTimeout)
	end := time.Since(start)
	if err != nil {
		return nil, err
	}
...
// grpc handler
// VM 배포
func (f *FxGateway) DeployVM(c context.Context, s *pb.CreateVMRequest) (*pb.Message, error) {
	deployVMConfig := &cmd.DeployVMConfig{
		VMNamespace:   f.conf.VMNamespace,
		FxWatcherPort: f.conf.FxWatcherPort,
		FxMeshPort:    f.conf.FxMeshPort,
	}
	ip, err := cmd.VMDeploy(s, f.kubeClient, deployVMConfig)
	if err != nil {
		return nil, err
	}
	return &pb.Message{Msg: ip}, nil
}
...
// grpc handler
// VM 삭제
func (f *FxGateway) DeleteVM(c context.Context, s *pb.DeleteVMRequest) (*pb.Message, error) {
	err := cmd.DeleteVM(s.VMName, f.conf.VMNamespace, f.kubeClient)
	if err != nil {
		return nil, err
	}
	return &pb.Message{Msg: "OK"}, nil
